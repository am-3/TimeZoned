...
def check_events(screen, screen_size):
    ...
    global city_num, city_name, city_name_addition, t_e, t_s
    e_w, e_h = screen_width / (city_num+2), screen_height / 30
...
def event_mouse(event, screen, screen_size):
    ...
    #y = []
    if event.button == 1 and e_w<mouse_x<(city_num+1)*e_w and 2*e_h<mouse_y:
        #y.append(mouse_y)
        y = mouse_y
...
def get_time():
    ...
    time_start = datetime.datetime(day.year,day.month,day.day,int(t_s//2),int(t_s%2)*30,tzinfo=tz)
    #time_end=str(day.year)+'-'+str(day.month)+'-'+str(day.day)+'-'+str(int(t_e//2))+'-'+str(int(t_e%2)*30)
    time_end = datetime.datetime(day.year,day.month,day.day,int(t_e//2),int(t_e%2)*30,tzinfo=tz)
    #time_end = datetime.datetime.strptime(time_end, "%Y-%m-%d-%H-%M-%Z")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        mouse_x,mouse_y == pygame.mouse.get_pos()
        if e_w<mouse_x<(city_num+1)*e_w and 2*e_h<mouse_y:
            pygame.mouse.set_cursor(*pygame.cursors.diamond)
        else:
            pygame.mouse.set_cursor(*pygame.cursors.arrow)
        choosen_city = int(mouse_x // e_w)
        try:
            tz = pytz.timezone(city_tz[choosen_city-1])
        except:
